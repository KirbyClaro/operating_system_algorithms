...
BTN_FG="#FFFFFF";ACCENT="#7C3AED";ACCENT_L="#F5F3FF";FONT="Segoe UI"

def run_algorithm(pages,capacity):
    frames={}; freq_map=defaultdict(list); min_freq=0; faults=hits=0; log=[]
    for page in pages:
        if page in frames:
            hits+=1; status="HIT"; f=frames[page]; freq_map[f].remove(page)
            frames[page]=f+1; freq_map[f+1].append(page)
            if not freq_map[min_freq]: min_freq+=1
            extra=f"Page {page} freq → {frames[page]}"
        else:
            faults+=1; status="FAULT"; extra=""
            if len(frames)==capacity:
                evicted=freq_map[min_freq].pop(0); del frames[evicted]
                extra=f"Evicted page {evicted} (freq={min_freq})"
            frames[page]=1; freq_map[1].append(page); min_freq=1
        log.append({"page":page,"frames":{p:frames[p] for p in frames},"status":status,"extra":extra})
    return faults,hits,log

class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.title("LFU Page Replacement"); self.configure(bg=BG)
    ...
    def _build_ui(self):
        ...
        tk.Label(bar,text="LFU  ·  Page Replacement",font=(FONT,18,"bold"),bg=ACCENT,fg=BTN_FG).pack()
        tk.Label(bar,text="Least Frequently Used  —  evicts the page with the lowest access count",font=(FONT,9),bg=ACCENT,fg="#ddd6fe").pack()
        ...
        for txt,w in [("#",4),("Page",6),("Frames (page:freq)",34),("Status",10)]:
    ...
    def _update_cells(self,frames_data,status):
        ...
        for i,(cell,lbl,sub) in enumerate(self._cells):
            if i<len(items):
                ...
                sub.config(text=f"×{freq}" if freq is not None else "",bg=bg)
            else:
