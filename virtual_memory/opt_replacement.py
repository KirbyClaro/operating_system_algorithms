...
BTN_FG="#FFFFFF";ACCENT="#D97706";ACCENT_L="#FFFBEB";FONT="Segoe UI"

def run_algorithm(pages, capacity):
    frames=[]; faults=hits=0; log=[]
    for idx, page in enumerate(pages):
        if page in frames:
            hits+=1; status="HIT"; extra=f"Page {page} already in memory"
        else:
            faults+=1; status="FAULT"; extra=""
            if len(frames) < capacity:
                frames.append(page)
            else:
                future=pages[idx+1:]; farthest=-1; victim=frames[0]
                for f in frames:
                    try: nxt=future.index(f)
                    except ValueError: victim=f; break
                    if nxt>farthest: farthest,victim=nxt,f
                frames[frames.index(victim)]=page
                extra=f"Evicted page {victim} (next use furthest)"
        log.append({"page":page,"frames":list(frames),"status":status,"extra":extra})
    return faults,hits,log

class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.title("Optimal Page Replacement"); self.configure(bg=BG)
    ...
    def _build_ui(self):
        ...
        tk.Label(bar,text="Optimal  ·  Page Replacement",font=(FONT,18,"bold"),bg=ACCENT,fg=BTN_FG).pack()
        tk.Label(bar,text="Bélády's Algorithm  —  evicts the page needed furthest in the future",font=(FONT,9),bg=ACCENT,fg="#fde68a").pack()
        ...
        tk.Button(row,text="  Start  ",command=self._on_start,font=(FONT,10,"bold"),bg=ACCENT,fg="#1e1e2e",relief="flat",bd=0,padx=14,pady=8,cursor="hand2").pack(side="left",padx=(10,0))
        ...
        self._btn_next=tk.Button(nav,text="Next  ▶",command=self._on_next,font=(FONT,10,"bold"),bg=ACCENT,fg="#1e1e2e",relief="flat",bd=0,padx=14,pady=8,width=10,cursor="hand2",state="disabled"); self._btn_next.pack(side="left",padx=(8,0))
    ...
    def _update_cells(self,frames_data,status):
        ...
        for i,(cell,lbl,sub) in enumerate(self._cells):
            if i<len(items):
                pg,freq=items[i]; active=pg==self._cur_page
    ...
    def _add_hist_row(self,idx,entry):
        ...
        fd=entry["frames"]; fs="  ".join(f"{p}:{f}" for p,f in fd.items()) if isinstance(fd,dict) else "  ".join(str(p) for p in fd)
