...
BTN_FG="#FFFFFF";ACCENT="#DC2626";ACCENT_L="#FEF2F2";FONT="Segoe UI"

def run_algorithm(pages,capacity):
    frames={}; order=0; faults=hits=0; log=[]
    for page in pages:
        if page in frames:
            hits+=1; status="HIT"; freq,ins=frames[page]; frames[page]=(freq+1,ins)
            extra=f"Page {page} freq → {frames[page][0]}"
        else:
            faults+=1; status="FAULT"; extra=""
            if len(frames)==capacity:
                victim=max(frames,key=lambda p:(frames[p][0],-frames[p][1]))
                extra=f"Evicted page {victim} (freq={frames[victim][0]})"
                del frames[victim]
            frames[page]=(1,order); order+=1
        log.append({"page":page,"frames":{p:frames[p][0] for p in frames},"status":status,"extra":extra})
    return faults,hits,log

class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.title("MFU Page Replacement"); self.configure(bg=BG)
    ...
    def _build_ui(self):
        ...
        tk.Label(bar,text="MFU  ·  Page Replacement",font=(FONT,18,"bold"),bg=ACCENT,fg=BTN_FG).pack()
        tk.Label(bar,text="Most Frequently Used  —  evicts the page with the highest access count",font=(FONT,9),bg=ACCENT,fg="#fecaca").pack()
