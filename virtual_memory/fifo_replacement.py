...
def run_algorithm(pages, capacity):
    ...
    for page in pages:
        if page in frame_set:
            hits += 1; status = "HIT"
            extra = f"Page {page} already in memory"
        else:
            faults += 1; status = "FAULT"; extra = ""
            if len(frames) == capacity:
                evicted = frames.popleft()
                frame_set.remove(evicted)
                extra = f"Evicted page {evicted}"
            frames.append(page); frame_set.add(page)
        log.append({"page": page, "frames": list(frames), "status": status, "extra": extra})
    return faults, hits, log

class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("FIFO Page Replacement")
        self.configure(bg=BG)
        self.geometry("860x660")
        self._log = []; self._step = -1; self._capacity = 3
        self._build_ui()

    def _build_ui(self):
        # Top bar
        bar = tk.Frame(self, bg=ACCENT, pady=16); bar.pack(fill="x")
        tk.Label(bar, text="FIFO  ·  Page Replacement", font=(FONT, 18, "bold"), bg=ACCENT, fg=BTN_FG).pack()
        tk.Label(bar, text="First-In, First-Out  —  oldest page is evicted first", font=(FONT, 9), bg=ACCENT, fg="#c7d2fe").pack()
        ...
        self._cells_frame = tk.Frame(outer, bg=BG); self._cells_frame.pack(anchor="w")
        self._cells = []
        ...
        self._btn_back = tk.Button(nav, text="◀  Back", command=self._on_back, font=(FONT, 10, "bold"), bg="#E5E7EB", fg=TEXT_PRI, relief="flat", bd=0, padx=14, pady=8, width=10, cursor="hand2", state="disabled")
        self._btn_back.pack(side="left")
        self._btn_next = tk.Button(nav, text="Next  ▶", command=self._on_next, font=(FONT, 10, "bold"), bg=ACCENT, fg=BTN_FG, relief="flat", bd=0, padx=14, pady=8, width=10, cursor="hand2", state="disabled")
        self._btn_next.pack(side="left", padx=(8,0))
        self._btn_reset = tk.Button(nav, text="Reset", command=self._on_reset, font=(FONT, 10, "bold"), bg="#E5E7EB", fg=TEXT_PRI, relief="flat", bd=0, padx=14, pady=8, width=7, cursor="hand2", state="disabled")
        ...
        for txt, w in [("#", 4), ("Page", 6), ("Frames after", 34), ("Status", 10)]:
        ...
        self._cwin = self._hist_canvas.create_window((0,0), window=self._hist_inner, anchor="nw")
        self._hist_inner.bind("<Configure>", lambda e: self._hist_canvas.configure(scrollregion=self._hist_canvas.bbox("all")))
        self._hist_canvas.bind("<Configure>", lambda e: self._hist_canvas.itemconfig(self._cwin, width=e.width))
    ...
    def _rebuild_cells(self, cap):
        ...
        for _ in range(cap):
            ...
            lbl = tk.Label(cell, text="—", font=(FONT, 20, "bold"), bg=SURFACE, fg=EMPTY_FG)
            lbl.place(relx=.5, rely=.45, anchor="center")
            sub = tk.Label(cell, text="", font=(FONT, 7), bg=SURFACE, fg=TEXT_SEC)
            sub.place(relx=.5, rely=.82, anchor="center")
            self._cells.append((cell, lbl, sub))

    def _update_cells(self, frames_data, status):
        items = list(frames_data.items()) if isinstance(frames_data, dict) else [(p, None) for p in frames_data]
        cbg = HIT_BG if status == "HIT" else FAULT_BG
        cfg = HIT_FG if status == "HIT" else FAULT_FG
        for i, (cell, lbl, sub) in enumerate(self._cells):
            if i < len(items):
                pg, freq = items[i]
                active = pg == self._cur_page
                bg = cbg if active else SURFACE; fg = cfg if active else TEXT_PRI
                cell.config(bg=bg); lbl.config(text=str(pg), bg=bg, fg=fg)
                sub.config(text=f"freq {freq}" if freq is not None else "", bg=bg)
            else:
                cell.config(bg=SURFACE); lbl.config(text="—", bg=SURFACE, fg=EMPTY_FG); sub.config(text="", bg=SURFACE)

    def _add_hist_row(self, idx, entry):
        bg = HIT_BG if entry["status"] == "HIT" else FAULT_BG
        fg = HIT_FG if entry["status"] == "HIT" else FAULT_FG
        frames_data = entry["frames"]
        fs = "  ".join(f"{p}:{f}" for p,f in frames_data.items()) if isinstance(frames_data, dict) else "  ".join(str(p) for p in frames_data)
    ...
    def _on_start(self):
        ...
        self._step = -1
        self._rebuild_cells(self._capacity)
        for w in self._hist_inner.winfo_children(): w.destroy()
        self._lbl_page.config(text=""); self._badge.config(text="", bg=BG)
        self._lbl_extra.config(text=""); self._lbl_counter.config(text=f"0 / {len(self._log)} steps")
        self._lbl_hits.config(text=""); self._lbl_faults.config(text="")
        self._btn_next.config(state="normal"); self._btn_back.config(state="disabled"); self._btn_reset.config(state="normal")

    def _on_next(self):
        if self._step < len(self._log) - 1:
            self._step += 1; self._render(self._step)
